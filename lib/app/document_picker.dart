...
class DocumentHelper {
  ...
  Future<File?> pickDeskTopImage(PickImageFromEnum type) async {
    final ImagePickerPlatform deskTopPicker = ImagePickerPlatform.instance;

    final PickedFile? photo;
    if (type == PickImageFromEnum.camera) {
      photo = await deskTopPicker.pickImage(source: ImageSource.camera);
    } else {
      photo = await deskTopPicker.pickImage(source: ImageSource.gallery);
    }
    if (photo == null) {
      print("photo");
      print(photo);
